...
export const AdminWithdrawals = () => {
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error || 'Failed to load withdrawals');
  ...
  const handleAction = async (id: string, status: TxStatus) => {
    try {
      ...
      const response = await fetch('/api/transactions', {
        ...
        body: JSON.stringify({ action: status === 'Rejected' ? 'reject' : 'approve', transactionId: id }),
      });
      const result = await response.json();
            ...
            {withdrawals.length === 0 ? (
              ...
              <tr key={req.id} className="hover:bg-white/[0.02] transition-colors">
                ...
                <td className="px-6 py-4">
                  ...
                  {req.instructions ? (
                    <div className="text-[10px] text-slate-400 max-w-[200px] truncate"><span className="text-orange-500">Instr:</span> {req.instructions}</div>
                  ) : (
                    <div className="flex flex-col gap-1 w-[200px]">
                      ...
                      {instructionTexts[req.id] && (
                        <button onClick={() => handleSendInstructions(req.id)} className="bg-orange-500/20 text-orange-500 px-2 py-1 flex items-center justify-center gap-1 rounded text-[10px] uppercase font-bold hover:bg-orange-500 hover:text-black">
